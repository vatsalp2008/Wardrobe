...
| F9 | 🟡 **Supabase** — `LiveSupabaseService` (anonymous auth + image upload → public URL) is implemented + key-gated (Phase 5). Activates when `SUPABASE_URL`/`SUPABASE_ANON_KEY` are set + buckets created (free; see SETUP.md). **Remaining:** full Core Data ↔ `wardrobe_items` row sync (cross-device) — auth + image hosting are done, DB row sync is the follow-on | Cross-device data sync | Create project (SETUP.md); then build row-sync |
